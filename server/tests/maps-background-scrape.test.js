...
test("mỗi ô phải cuộn lấy URL rồi mở tuần tự từng URL trước khi sang ô kế tiếp", () => {
  ...
  assert.match(
    enrich,
    /enrichRunPromise = null;[\s\S]*continueGridAfterEnrich\(nextIndex\)/,
  ...
  assert.match(enrich, /return completeCellAfterEnrich\(cellIndex, enrichGeneration\)/);
...
test("feed instance cũ được đọc trước khi đổi vùng và chỉ bắt buộc đổi ở ô mới", () => {
  const runCell = section("async function runGridCell", "function getEnrichCheckpointKey");
  const readSignatureAt = runCell.indexOf('sendMapsMessageWithTimeout("GET_FEED_SIGNATURE"');
...
test("content Maps phiên bản cũ bị từ chối cả ở lần PING cuối", async () => {
  const source = section(
    ...
    "async function sendMapsMessage"
  );
  ...
  try {
    await Promise.race([
      ...
      new Promise((_, reject) => {
        searchTimeout = setTimeout(() => reject(new Error("deadlock enrich nhiều ô")), 2000);
      })
    ]);
  } finally {
...
test("danh sách dài được checkpoint theo chunk rồi cuộn tiếp cùng ô", () => {
  ...
  assert.match(runCell, /let resumeFromCurrent = continuingSameCell/);
  assert.match(runCell, /resumeFromCurrent,/);
...
test("START chỉ chạy Maps sau khi checkpoint đầu tiên đã lưu thành công", () => {
  const startSearch = section("async function handleStartSearch", "async function handleResumeSearch");
  const startRescan = section("async function handleStartRescan", "async function enrichRescanPlace");

  const searchCheckpointAt = startSearch.indexOf("await persistScrapeCheckpoint()");
  const searchGridAt = startSearch.indexOf("runGridCell(0)", searchCheckpointAt);
  assert.ok(searchCheckpointAt >= 0 && searchGridAt > searchCheckpointAt);
  assert.match(startSearch, /if \(!initialCheckpointSaved\)[\s\S]*Không lưu được trạng thái tìm kiếm/);

  const rescanCheckpointAt = startRescan.indexOf("await persistRescanCheckpoint()");
  const rescanLoopAt = startRescan.indexOf("doRescan(params)", rescanCheckpointAt);
  assert.ok(rescanCheckpointAt >= 0 && rescanLoopAt > rescanCheckpointAt);
...
test("chunk có tiến triển tiếp tục mà không tăng failure retry", () => {
  const progress = section("function updateCellListProgress", "function clearCellListProgress");
  const retry = section("async function retryIncompleteGridCell", "function isCompleteCellResult");
  const continuationAt = retry.indexOf("if (canContinueChunk)");
  const incrementAt = retry.indexOf("retryCounts[cellIndex] = nextAttempt");

  assert.doesNotMatch(progress, /result\.newPlacesCount/);
  assert.match(progress, /stagedNewPlacesCount/);
  assert.match(
    retry,
    /reason === "chunk_budget" \|\|[\s\S]*reason === "renderer_suspended" \|\|[\s\S]*reason === "message_timeout"/
  );
  assert.match(retry, /noGrowthMs[\s\S]*CELL_LIST_STALL_RETRY_MS/);
  assert.ok(continuationAt >= 0 && incrementAt > continuationAt);
});

test("enrich watchdog invalid generation, reload barrier rồi resume ô cũ", async () => {
  const source = section("async function recoverStalledEnrich", "async function maybeRecoverStalledScrape");
  const calls = [];
  const staleTask = Promise.resolve();
  const scrapeState = {
    running: true,
    phase: "enrich",
    mapsTabId: 7,
  ...
  const context = vm.createContext({
    ...
    pointsFinalized: false,
    maybeFocusMapsTabForStall: async () => calls.push("focus"),
    persistScrapeCheckpoint: async () => calls.push("checkpoint"),
    bgLog: () => {},
    cancelActiveEnrichOperation: async () => (calls.push("cancel"), { reloaded: false }),
    reloadMapsAfterUnsettledEnrich: async () => calls.push("reload"),
    notifyProgress: () => {},
    calcProgressPercent: () => 50,
    runEnrichPhase: () => (calls.push("resume"), Promise.resolve()),
    abortSearch: async () => {},
    Date,
    Math
  });
  vm.runInContext(`${source}\nthis.recoverStalledEnrich = recoverStalledEnrich;`, context);

  assert.equal(await context.recoverStalledEnrich(300000), true);
  assert.equal(scrapeState._enrichGeneration, 5);
  assert.equal(scrapeState._activeEnrichOpId, "");
  assert.deepEqual(calls, ["focus", "checkpoint", "cancel", "reload", "checkpoint", "resume"]);
});

test("enrich sang grid đặt lại đồng hồ no-data cho ô kế tiếp", async () => {
  const clock = section("function markMapsDataActivity", "function markRescanDataActivity");
  const complete = section(
    "async function completeCellAfterEnrich",
    "function continueGridAfterEnrich"
  );
  const scrapeState = {
    running: true,
    phase: "enrich",
    ...
    _cellContinueFlags: {},
    _enrichGeneration: 7,
    _lastRecoveryFocusDataAt: 1234,
    _lastSoftRecoveryAt: 5678
  };
  ...
  const context = vm.createContext({
    Date: FixedDate,
    scrapeState,
    lastScrapeProgressAt: 1000,
    clearCellListProgress: () => {},
    clearPendingCellPlaces: () => {},
    getFinalResultsList: () => [{ name: "Đã enrich" }],
    calcProgressPercent: () => 50,
    notifyProgress: () => {},
    scheduleSyncSnapshot: () => {},
    persistScrapeCheckpoint: async () => true,
    sendMapsMessage: async () => ({ success: true }),
    handleScrapeComplete: async () => {},
    beginOperationTransition: () => null,
    endOperationTransition: () => {},
    closeMapsTabSafely: async () => {},
    resetScrapeState: async () => {},
  ...
  vm.runInContext(
    `${clock}\n${complete}\nthis.completeCellAfterEnrich = completeCellAfterEnrich;`,
    context
  );

  const nextIndex = await context.completeCellAfterEnrich(0, 7);

  assert.equal(nextIndex, 1);
  assert.equal(scrapeState.phase, "grid");
  assert.equal(context.lastScrapeProgressAt, 900000);
  assert.equal(scrapeState._lastRecoveryFocusDataAt, 0);
  assert.equal(scrapeState._lastSoftRecoveryAt, 0);
});

test("enrich watchdog không tiếp tục khi phase, ô hoặc generation đổi qua await", async () => {
  const source = section("async function recoverStalledEnrich", "async function maybeRecoverStalledScrape");

  async function runCase(mutateAt) {
    const calls = [];
    const scrapeState = {
      running: true,
      runId: "run-owner",
    ...
    const context = vm.createContext({
      ...
      enrichRunPromise: staleTask,
      pointsFinalized: false,
      maybeFocusMapsTabForStall: async () => {
        calls.push("focus");
        if (mutateAt === "focus") scrapeState.phase = "grid";
      },
      persistScrapeCheckpoint: async () => calls.push("checkpoint"),
      bgLog: () => {},
      cancelActiveEnrichOperation: async () => {
        calls.push("cancel");
        if (mutateAt === "cancel") scrapeState.gridIndex = 1;
        return { settled: true, reloaded: mutateAt !== "reload" };
      },
      reloadMapsAfterUnsettledEnrich: async () => {
        calls.push("reload");
        if (mutateAt === "reload") scrapeState._enrichGeneration += 1;
      },
      notifyProgress: () => calls.push("notify"),
      calcProgressPercent: () => 50,
      runEnrichPhase: () => {
        calls.push("resume");
        return Promise.resolve();
      },
      abortSearch: async () => calls.push("abort"),
      Date,
      Math
    });
    vm.runInContext(`${source}\nthis.recoverStalledEnrich = recoverStalledEnrich;`, context);

    await context.recoverStalledEnrich(300000);
    await new Promise((resolve) => setImmediate(resolve));
    return { calls, scrapeState };
  }

  const phaseChanged = await runCase("focus");
  assert.deepEqual(phaseChanged.calls, ["focus"]);
  assert.equal(phaseChanged.scrapeState._enrichGeneration, 4);

  const cellChanged = await runCase("cancel");
  assert.ok(cellChanged.calls.includes("cancel"));
  assert.equal(cellChanged.calls.includes("reload"), false);
  assert.equal(cellChanged.calls.includes("resume"), false);
  assert.equal(cellChanged.scrapeState.gridIndex, 1);

  const generationChanged = await runCase("reload");
  assert.ok(generationChanged.calls.includes("reload"));
  assert.equal(generationChanged.calls.includes("resume"), false);
  assert.equal(generationChanged.calls.filter((call) => call === "checkpoint").length, 1);
});

test("renderer hoặc message timeout có checkpoint tiếp tục mà không tiêu hard retry", async () => {
  const source = section("async function retryIncompleteGridCell", "function isCompleteCellResult");

  for (const reason of ["renderer_suspended", "message_timeout"]) {
    const calls = [];
    const scrapeState = {
      running: true,
      runId: `run-${reason}`,
      ...
      gridIndex: 0,
      totalCells: 2,
      _cellRetryCounts: { 0: 2 },
      _cellRecoveryCounts: {},
      _cellContinueFlags: {},
      _cellRestartFlags: {},
      _cellListProgress: {
        0: { urlCount: 1, scrollTop: 400, scrollHeight: 1600, noGrowthMs: 0 }
      },
      _scheduledCellRetry: ""
    };
    const context = vm.createContext({
      scrapeState,
      CELL_LIST_STALL_RETRY_MS: 300000,
      MAX_INCOMPLETE_CELL_RETRIES: 3,
      MAX_CELL_HARD_RECOVERIES: 1,
      getPendingCellPlaces: () => [{ name: "URL đã checkpoint" }],
      persistScrapeCheckpoint: async () => calls.push("checkpoint"),
      calcProgressPercent: () => 25,
      notifyProgress: () => {},
      notifyPopup: () => {},
      bgLog: () => {},
      sleep: async () => {},
      runGridCell: async () => calls.push("run-grid"),
      clearCellListProgress: () => calls.push("clear-progress"),
      chrome: { tabs: {} },
      waitTabComplete: async () => {},
      ensureMapsContentReady: async () => true,
      focusMapsTabForRecovery: async () => {},
      markMapsControlledActivity: () => {},
      abortSearch: async () => calls.push("abort")
    });
    vm.runInContext(`${source}\nthis.retryIncompleteGridCell = retryIncompleteGridCell;`, context);

    const continued = await context.retryIncompleteGridCell(0, reason, {
      grew: true,
      urlGrew: true,
      scrollTop: 400,
      scrollHeight: 1600,
      lastItemKey: "place:last",
      noGrowthMs: 0,
      resumeFromCurrent: reason !== "message_timeout"
    });

    assert.equal(continued, true, reason);
    assert.equal(scrapeState._cellContinueFlags[0], true, reason);
    assert.equal(scrapeState._cellRetryCounts[0], 0, reason);
    assert.equal(
      scrapeState._cellRestartFlags[0] === true,
      reason === "message_timeout",
      reason
    );
    assert.deepEqual(calls, ["checkpoint", "run-grid"], reason);
  }
});

test("restore checkpoint giữa chunk giữ continuation và lease cũ để dựng isolation barrier", () => {
  ...
  const context = vm.createContext({
    ...
    scrapeState,
    placesToMap: (places) =>
      new Map((places || []).map((place, index) => [place.href || String(index), place])),
    activeMapsCellListToken: null,
    currentSearch: null,
    lastScrapeProgressAt: 0,
    Date
  ...
  const restored = context.restoreScrapeStateFromCheckpoint({
    ...
    runId: "run-restored",
    cellGeneration: 11,
    searchParams: { searchId: "run-restored", keyword: "trà đá" },
    mapsTabId: 17,
    gridIndex: 1,
    totalCells: 3,
    phase: "grid",
    ...
    pendingCellIndex: 1,
    pendingCellPlaces: [
      { name: "Đã checkpoint", href: "https://www.google.com/maps/place/a" }
    ],
    cellContinueFlags: {},
    cellRestartFlags: {},
    cellListProgress: {
      1: {
        ...
        scrollHeight: 2200,
        lastItemKey: "place:a",
        contentInstanceId: "content-before-worker-sleep"
      }
    }
  });

  assert.equal(restored, true);
  assert.equal(scrapeState._cellContinueFlags[1], true);
  assert.deepEqual(plain(scrapeState._cellResumeLeases[1]), {
    runId: "run-restored",
    cellGeneration: 11
  });
  assert.equal(scrapeState._cellListProgress[1].contentInstanceId, "content-before-worker-sleep");
});

test("continuation restore dựng abort/reload barrier trước request mới và vẫn giữ pending globalSeen", () => {
  const barrier = section(
    "async function abortCellListLeaseForContinuation",
    "async function retryIncompleteGridCell"
  );
  const runCell = section("async function runGridCell", "function getEnrichCheckpointKey");

  const boundedAbortAt = barrier.indexOf("sendTabMessageWithTimeout(");
  const reloadAt = barrier.indexOf("reloadMapsAfterUnsettledEnrich", boundedAbortAt);
  assert.ok(boundedAbortAt >= 0 && reloadAt > boundedAbortAt);
  assert.match(barrier, /return \{ settled: false, reloaded: true/);

  const pendingAt = runCell.indexOf("...pendingCellPlaces");
  const resumeLeaseAt = runCell.indexOf("const resumeLease");
  const abortAt = runCell.indexOf("abortCellListLeaseForContinuation", resumeLeaseAt);
  const readyAt = runCell.indexOf("ensureMapsContentReady", abortAt);
  const requestAt = runCell.indexOf('"SCRAPE_CELL_LIST"', readyAt);
  const resumeFlagAt = runCell.indexOf("resumeFromCurrent,", requestAt);

  assert.ok(pendingAt >= 0, "pending URL phải tiếp tục nằm trong globalSeen khi restart DOM");
...
test("PING treo có timeout và ensureMapsContentReady trả về thay vì chờ vô hạn", async () => {
  ...
  const context = vm.createContext({
    ...
    chrome: {
      ...
      scripting: { executeScript: async () => { injectCount += 1; } }
    },
    markMapsControlledActivity: () => {},
    waitTabComplete: async () => {},
    sleep: async () => {},
  ...
  try {
    ready = await Promise.race([
      context.ensureMapsContentReady(17),
      new Promise((_, reject) => {
        guardTimer = nativeSetTimeout(() => reject(new Error("PING không có timeout")), 200);
      })
    ]);
  } finally {
  ...
  assert.equal(pingCount, 1, "renderer treo chỉ được probe hữu hạn một lần trong lượt recovery");
  assert.equal(injectCount, 0, "không xếp thêm lệnh inject khi renderer chưa phản hồi");
});

test("abandon main không force-release power khi rescan vẫn chạy", async () => {
  const source = section("async function abandonActiveSearch", "async function ensureReadyForNewSearch");
  const releases = [];
  const context = vm.createContext({
    scrapeState: { running: false, searchParams: null },
    rescanState: { running: true },
    beginOperationTransition: () => "transition",
    endOperationTransition: () => {},
  ...
  vm.runInContext(`${source}\nthis.abandonActiveSearch = abandonActiveSearch;`, context);

  assert.equal((await context.abandonActiveSearch()).success, true);
...
test("continuation park khi renderer chưa ready và keepalive chỉ resume sau preflight", async () => {
  ...
  const parkedState = {
    ...
    cellGeneration: 4,
    completedCells: new Set(),
    searchParams: { keyword: "trà đá" },
    gridPoints: [{ lat: 21.02, lng: 105.81, cellId: "cell-0" }],
    viewportM: 1200,
    mergedPlaces: new Map(),
    _cellContinueFlags: { 0: true },
    _cellRestartFlags: {},
    _cellResumeLeases: {},
    _cellRetryCounts: {},
    _cellRecoveryCounts: {},
  ...
  const parkContext = vm.createContext({
    scrapeState: parkedState,
    buildMapsUrl: () => "https://www.google.com/maps/search/tra-da",
    getPendingCellPlaces: () => [{ name: "Đã checkpoint" }],
    buildGlobalSeenKeys: () => ["pending:a"],
    sendMapsMessageWithTimeout: async (action) => {
      parkCalls.push(`send:${action}`);
      return { success: true, signature: "feed", instanceId: "instance" };
    },
    notifyProgress: () => {},
    calcProgressPercent: () => 25,
    buildProgressText: () => "progress",
    sleep: async () => {},
    ensureMapsContentReady: async () => {
      parkCalls.push("preflight:false");
      return false;
    },
    persistScrapeCheckpoint: async () => parkCalls.push("persist"),
    ensureDurableWorkAlarm: async () => parkCalls.push("alarm"),
    bgLog: () => {},
    Date,
    Math
  });
  vm.runInContext(
    `${continuation}\n${runCell}\nthis.runGridCell = runGridCell;`,
    parkContext
  );

  await parkContext.runGridCell(0);

  assert.equal(parkedState._pendingGridContinuation, 0);
  assert.equal(parkedState._cellContinueFlags[0], true);
  assert.deepEqual(parkCalls, [
    "send:GET_FEED_SIGNATURE",
    "preflight:false",
    "persist",
    "alarm"
  ]);
  assert.equal(parkCalls.some((call) => call === "send:SCRAPE_CELL_LIST"), false);

  const resumeCalls = [];
  const readySequence = [false, true];
  const resumeState = {
    running: true,
    phase: "grid",
    mapsTabId: 17,
    webTabId: null,
    totalCells: 1,
    _pendingGridContinuation: 0,
    _mapsCellWorkActive: false,
    _mapsCellListActive: false,
    _scheduledCellRetry: ""
  };
  const resumeContext = vm.createContext({
    scrapeState: resumeState,
    webPushTick: 0,
    ...
    stopScrapeKeepAlive: () => resumeCalls.push("stop"),
    pingMapsTabWake: () => {},
    maybeFocusMapsTabForStall: async () => {},
    getFinalResultsList: () => [],
    ensureWebSyncedToResults: async () => true,
    reconcileWebCountWithExtension: async () => true,
    persistScrapeCheckpoint: async () => resumeCalls.push("persist"),
    ensureMapsContentReady: async () => {
      const ready = readySequence.shift();
      resumeCalls.push(`preflight:${ready}`);
      return ready;
    },
    runGridCell: async (cellIndex) => resumeCalls.push(`run:${cellIndex}`),
    maybeRecoverStalledScrape: async () => resumeCalls.push("watchdog"),
    abortSearch: async () => resumeCalls.push("abort"),
    pointsFinalized: false,
    Date
  });
  vm.runInContext(
    `${continuation}\n${keepalive}\nthis.scrapeKeepAliveTick = scrapeKeepAliveTick;`,
    resumeContext
  );

  await resumeContext.scrapeKeepAliveTick();
  assert.equal(resumeState._pendingGridContinuation, 0);
  assert.equal(resumeCalls.includes("run:0"), false);
  assert.equal(resumeCalls.includes("abort"), false);

  await resumeContext.scrapeKeepAliveTick();
  assert.equal(resumeState._pendingGridContinuation, -1);
  assert.equal(resumeCalls.filter((call) => call === "run:0").length, 1);
  assert.equal(resumeCalls.includes("abort"), false);
