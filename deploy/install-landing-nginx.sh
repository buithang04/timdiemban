#!/usr/bin/env bash
# Deploy landing lên findmap.vn — chạy với quyền sudo một lần:
...
cp "$ROOT/deploy/findmap.vn.conf" /etc/nginx/sites-available/findmap.vn
...
echo "==> Kiểm tra nhanh"
echo -n "Guest / → "
curl -s http://127.0.0.1/ -H 'Host: findmap.vn' | grep -o '<title>[^<]*' || true
echo -n "Logged-in / → "
curl -s http://127.0.0.1/ -H 'Host: findmap.vn' -H 'Cookie: findmap_session=1' | grep -o '<title>[^<]*' || true
echo ""
echo "Done. URL chính: https://findmap.vn/"
