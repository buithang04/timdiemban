(function () {
  ...
  function links() {
    return document.querySelectorAll("[data-winmap-nav]");
  }
  ...
  async function refresh(status) {
    ...
    try {
      const response = await fetch("/api/integrations/winmap/status?verify=1", {
  ...
  window.FindmapWinmapNav = { refresh, setLinked };
  setLinked(false);
